...
import http, { syncHttp } from './http';
...
export async function syncCard(): Promise<CodefSyncResult> {
  const res = await syncApi.post<ApiResponse<CodefSyncResult>>('/api/codef/sync/card');
  return res.data.data;
}

// 증권 종합자산 동기화 — POST /api/codef/sync/stock
export async function syncStock(): Promise<CodefSyncResult> {
  const res = await syncApi.post<ApiResponse<CodefSyncResult>>('/api/codef/sync/stock');
  return res.data.data;
}
